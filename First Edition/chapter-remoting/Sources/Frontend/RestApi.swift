...
public struct BoxOfficeApi {
  ...
  public init(
    createEvent: @escaping (String, Int) async throws -> Event,
    getEvents: @escaping () async throws -> [Event],
    getEvent: @escaping (String) async throws -> Event?,
    cancelEvent: @escaping (String) async throws -> Void,
    requestTickets: @escaping (String, Int) async throws -> Tickets?
  ) {
    self.createEvent = createEvent
    self.getEvents = getEvents
    self.getEvent = getEvent
    self.cancelEvent = cancelEvent
    self.requestTickets = requestTickets
  }
}
